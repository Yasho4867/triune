...
def create_desktop_shortcut(hw: HardwareProfile) -> None:
    ...
    launch_bat = WORKSPACE_ROOT / "launch.bat"
    launch_sh = WORKSPACE_ROOT / "launch.sh"

    # Create root launch.bat
...
"""
    launch_bat.write_text(launch_bat_content, encoding="utf-8")

    # Create root launch.sh
    launch_sh_content = f"""#!/usr/bin/env bash
cd "$(dirname "$0")"
...
"""
    launch_sh.write_text(launch_sh_content, encoding="utf-8")
    try:
        launch_sh.chmod(0o755)
    except Exception:
        pass

    log_success("Created 1-click launch scripts: launch.bat & launch.sh")
...
$Shortcut.TargetPath = "{launch_bat}"
...
def main() -> None:
    ...
    parser.add_argument("--venv", default=".venv", help="Target virtual environment directory (default: .venv)")
    ...
    venv_path = WORKSPACE_ROOT / args.venv
    ...
    print(f"  • Windows: Double-click {BOLD}launch.bat{RESET} or desktop icon 'Triune Studio'")
    print(f"  • Linux/macOS: Run {BOLD}./launch.sh{RESET}")
